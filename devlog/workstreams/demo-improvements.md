...
## Follow-up: judge-facing README

Owner: integration agent. Scope: README, optional demo setup guide, related documentation links and this handoff. Dependencies: existing demo target, Compose configuration and UI labels. Acceptance: one README startup command, a clear judge walkthrough, optional setup separated from the main path, valid documentation links and `make check`. Status: complete. README has one startup command and a four-step judge walkthrough; optional setup lives in [the demo setup guide](../../docs/demo-setup.md). `make check` passes (65 CPU tests, lint/type checks, documentation checks and production build). Startup behavior was cross-checked against Makefile/Compose with a dry run; no deployment restart or browser/database integration run was needed for these documentation-only changes. Existing unrelated changes in `devlog/current.md` were preserved.
