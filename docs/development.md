...
For the one-command demo and click-by-click walkthrough, see the [quickstart](../README.md#quickstart). `make demo` builds and waits for service health, preferring prepared demos when present. Use `make demo DEMO_DIR=./tests/fixtures` to explicitly select synthetic data. The browser is localhost:8080. Run `docker compose logs --tail=100 api worker` to diagnose startup. Missing mounts fail clearly; source changes require a new analysis. `docker compose down` preserves database history. Deleting the named volume destroys all reports and reviews; it is not part of normal restart or validation.
